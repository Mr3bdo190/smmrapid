...
// The profile screen saves the display name here. Only `name` is writable — never email,
// role, balance or keys — and it is length-validated before it reaches the database.
app.put('/api/client/me', requireAuth, async (req: any, res) => {
  try {
    const name = String(req.body?.name ?? '').trim().replace(/\s+/g, ' ');
    if (name.length < 2 || name.length > 80) {
      return apiError(res, 400, 'Enter a display name between 2 and 80 characters.', 'INVALID_NAME');
    }
    const [updated] = await db.update(users).set({ name }).where(eq(users.id, req.dbUser.id)).returning();
    if (!updated) return apiError(res, 404, 'User not found', 'NOT_FOUND');
    await audit(req.dbUser.id, 'UPDATE_PROFILE', 'USER', req.dbUser.id, undefined, String(req.dbUser.name || ''), name);
    res.json(publicUser(updated));
  } catch (e: any) {
    return internalFailure(res, e, { kind: 'generic', code: 'PROFILE_UPDATE_FAILED', status: 500, route: '/api/client/me', message: 'Profile update failed' });
  }
});
...
app.post('/api/client/orders', orderLimiter, requireAuth, async (req: any, res) => {
  try {
    ...
    await db.transaction(async tx => {
      ...
      const [o] = await tx.insert(orders).values({ userId: req.dbUser.id, serviceId: service.id, link: String(req.body.link).trim(), quantity: q, charge: finalCharge.toFixed(4), cost: calculateProviderCost(service, q).toFixed(4), status: 'Pending' }).returning();
      orderId = o.id;
