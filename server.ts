...
async function requestRefillForOrder(orderId: string, userId: string) {
  ...
  const result = await new ProviderClient(provider.apiUrl, decryptSecret(provider.apiKey)).refill(o.providerOrderId);
...
async function requestCancelForOrder(orderId: string, userId: string) {
  ...
  const result = await new ProviderClient(provider.apiUrl, decryptSecret(provider.apiKey)).cancel(o.providerOrderId);
  if (result?.error) {
...
app.post('/api/client/orders', orderLimiter, requireAuth, async (req: any, res) => {
  try {
    ...
    await db.transaction(async tx => {
      ...
      const [o] = await tx.insert(orders).values({ userId: req.dbUser.id, serviceId: service.id, link: source.link.trim(), quantity: q, charge: finalCharge.toFixed(4), cost: calculateProviderCost(service, q).toFixed(4), status: 'Pending' }).returning();
      orderId = o.id;
...
app.get('/api/admin/affiliates',requireAuth,requireAdmin,async(_req:any,res:any)=>{try{
  const affiliates=await db.select({id:users.id,name:users.name,email:users.email,referralCode:users.referralCode,createdAt:users.createdAt}).from(users).where(sql`${users.referralCode} is not null`).orderBy(desc(users.createdAt));
  const clickCounts=await db.select({code:referralClicks.referralCode,count:sql<number>`count(*)`}).from(referralClicks).groupBy(referralClicks.referralCode);
  const referredRows=await db.select({affiliateId:users.referredBy,referredId:users.id}).from(users).where(sql`${users.referredBy} is not null`);
  const paidRows=await db.select({userId:payments.userId,amount:payments.amount}).from(payments).where(eq(payments.status,'Approved'));
  const commissionRowsRaw=await db.select({id:affiliateCommissions.id,affiliateId:affiliateCommissions.affiliateId,referredUserId:affiliateCommissions.referredUserId,paymentId:affiliateCommissions.paymentId,amount:affiliateCommissions.amount,createdAt:affiliateCommissions.createdAt}).from(affiliateCommissions).orderBy(desc(affiliateCommissions.createdAt)).limit(100);
  const commissionUserIds=Array.from(new Set(commissionRowsRaw.flatMap((c:any)=>[c.affiliateId,c.referredUserId])));
  const commissionUsers=commissionUserIds.length?await db.select({id:users.id,email:users.email}).from(users).where(inArray(users.id,commissionUserIds)):[];
  const commissionEmails=new Map(commissionUsers.map((u:any)=>[u.id,u.email]));
  const commissionRows=commissionRowsRaw.map((c:any)=>({...c,affiliateEmail:commissionEmails.get(c.affiliateId)||'-',referredEmail:commissionEmails.get(c.referredUserId)||'-'}));
  const clicks=new Map(clickCounts.map((x:any)=>[String(x.code),Number(x.count||0)]));
  const referredBy=new Map<string,string[]>(); for(const r of referredRows){if(!r.affiliateId)continue;const a=referredBy.get(r.affiliateId)||[];a.push(r.referredId);referredBy.set(r.affiliateId,a);}
  const paidByUser=new Map<string,number>(); for(const p of paidRows)paidByUser.set(p.userId,(paidByUser.get(p.userId)||0)+num(p.amount));
  const byAffiliate=new Map<string,number>(); for(const c of commissionRows)byAffiliate.set(String(c.affiliateId),(byAffiliate.get(String(c.affiliateId))||0)+num(c.amount));
  const rows=affiliates.map((a:any)=>{const ids=referredBy.get(a.id)||[];const paid=ids.filter(id=>paidByUser.has(id));const deposits=ids.reduce((sum,id)=>sum+(paidByUser.get(id)||0),0);return {id:a.id,name:a.name,email:a.email,referralCode:a.referralCode,createdAt:a.createdAt,clicks:clicks.get(String(a.referralCode))||0,signups:ids.length,paidReferrals:new Set(paid).size,referralDeposits:money(deposits),totalCommission:money(byAffiliate.get(a.id)||0)};});
  const summary={clicks:rows.reduce((n,r)=>n+r.clicks,0),signups:rows.reduce((n,r)=>n+r.signups,0),deposited:money(rows.reduce((n,r)=>n+num(r.referralDeposits),0)),commissions:money(rows.reduce((n,r)=>n+num(r.totalCommission),0))};
  res.json({summary,affiliates:rows,recentCommissions:commissionRows.map((c:any)=>({id:c.id,affiliateEmail:c.affiliateEmail||'-',referredEmail:c.referredEmail||'-',paymentId:c.paymentId,amount:c.amount,createdAt:c.createdAt}))});
}catch(e:any){logSystemError('error', 'Failed to load affiliate control center', e?.message || String(e)); apiError(res, 500, 'Internal server error', 'INTERNAL_ERROR');}});
...
app.get('/api/client/notifications', requireAuth, async (req: any, res: any) => {
  ...
  } catch (e:any) { logSystemError('error', 'Failed to load notifications', e.message || String(e)); apiError(res, 500, 'Internal server error', 'INTERNAL_ERROR'); }
});
...
app.get('/api/admin/users/export',adminLimiter,requireAuth,requireAdmin,async(req:any,res)=>{
  try {
    ...
    res.setHeader('Cache-Control','no-store');
    res.send(csvContent);
  } catch(e:any) { logSystemError('error', 'Failed to export users', e?.message || String(e)); apiError(res, 500, 'Internal server error', 'INTERNAL_ERROR'); }
});
...
app.post('/api/admin/providers/:id/sync',adminLimiter,requireAuth,requireAdmin,async(req:any,res:any)=>{
  try{
    ...
    const data=await new ProviderClient(p.apiUrl,decryptSecret(p.apiKey)).services();
...
app.get('/api/admin/orders/export',adminLimiter,requireAuth,requireAdmin,async(req:any,res)=>{
  try {
    ...
    res.setHeader('Cache-Control','no-store');
    res.send(csvContent);
  } catch(e:any) { logSystemError('error', 'Failed to export orders', e?.message || String(e)); apiError(res, 500, 'Internal server error', 'INTERNAL_ERROR'); }
});
...
app.get('/api/client/raffles', requireAuth, async (req:any,res:any) => {
  try {
    ...
    res.json(out);
  } catch (e:any) { logSystemError('error', 'Failed to load raffles', e?.message || String(e)); apiError(res, 500, 'Internal server error', 'INTERNAL_ERROR'); }
});
...
app.post('/api/client/raffles/:id/buy',orderLimiter, requireAuth, async (req:any,res:any) => {
  try {
    ...
    await db.transaction(async tx=>{
      ...
      await tx.insert(walletLedger).values({id:crypto.randomUUID(),userId:u.id,amount:(-cost).toFixed(4),type:'debit',description:`Raffle tickets: ${r.title}`,referenceId:r.id,createdAt:new Date()});
      const ticketRows=Array.from({length:qty},()=>({id:crypto.randomUUID(),raffleId:r.id,userId:u.id}));
      await tx.insert(raffleTickets).values(ticketRows); result={success:true,qty,cost,totalTickets:Number(cnt.count)+qty,userTickets:Number(mine.count)+qty};
    }); res.json(result);
...
app.get('/api/client/shortlinks', requireAuth, async (req:any,res:any) => {
  ...
  catch(e:any){logSystemError('error', 'Failed to load rewards', e?.message || String(e)); apiError(res, 500, 'Internal server error', 'INTERNAL_ERROR');}
});
...
// Supports both POST (JSON body) and GET (query parameters) for browser testing.
const handleSmmApi = async (req, res) => { try {
  const source = req.method === 'GET' ? req.query : req.body;
  const key = String(source?.key || ''); if (!key) return apiError(res, 401, 'Missing API key', 'INVALID_API_KEY');
  const u = await db.query.users.findFirst({ where: and(eq(users.status, 'active'), sql`${users.apiKeyHash} = ${hashApiKey(key)}`) });
  if (!u) return apiError(res, 401, 'Invalid API key', 'INVALID_API_KEY');
  const action = String(source?.action || '');if(action==='balance')return res.json({balance:u.balance,currency:(process.env.CURRENCY||'USD')});if(action==='services'){const rows=await db.query.services.findMany({where:eq(services.status,'active'),with:{category:true}});return res.json(rows.filter(s=>s.category?.status==='active').map(s=>({service:s.id,name:s.name,rate:s.pricePer1k,min:s.minQuantity,max:s.maxQuantity,category:s.category?.name||'',refill:s.refillable,cancel:s.cancelable})));}
if(action==='status'){
  const single=source?.order!==undefined;
  if(single){const o=await db.query.orders.findFirst({where:and(eq(orders.id,String(source.order||'')),eq(orders.userId,u.id))});if(!o)return apiError(res,404,'Order not found','NOT_FOUND');return res.json({order:o.id,status:o.status,charge:o.charge,start_count:o.startCount,remains:o.remains,currency:(process.env.CURRENCY||'USD')});}
  const ids=String(source?.orders||'').split(',').map(s=>s.trim()).filter(Boolean).slice(0,100);
...
if(action==='add'){const link=typeof source.link==='string'?source.link.trim():source.link;const {service,q,charge}=await validateOrderInput(source.service,link,source.quantity);let id='';await db.transaction(async tx=>{await debitWallet(tx,u.id,charge,'API order',undefined);const [o]=await tx.insert(orders).values({userId:u.id,serviceId:service.id,link,quantity:q,charge:charge.toFixed(4),cost:calculateProviderCost(service,q).toFixed(4),status:'Pending'}).returning();id=o.id;});placeOrderToProvider(id).catch(console.error);return res.json({order:id});}
if(action==='refill'){
  const single=source?.order!==undefined;
  if(single){try{const r=await requestRefillForOrder(String(source.order||''),u.id);return res.json({refill:r.id});}catch(e:any){return res.json({error:e.message||'Refill failed'});}}
  const ids=String(source?.orders||'').split(',').map(s=>s.trim()).filter(Boolean).slice(0,100);
  if(!ids.length)return apiError(res,400,'Provide order or orders','VALIDATION_ERROR');
...
if(action==='refill_status'){
  const single=source?.refill!==undefined;
  ...
  const ids=String(source?.refills||'').split(',').map(s=>s.trim()).filter(Boolean).slice(0,100);
...
if(action==='cancel'){
  const ids=String(source?.orders||req.body?.order||'').split(',').map(s=>s.trim()).filter(Boolean).slice(0,100);
...
app.all(['/api/v1', '/api/v2'], apiLimiter, handleSmmApi);
...
function validateEnv() {
  ...
  if (isProd) {
    if (!process.env.PROVIDER_ENCRYPTION_KEY) missing.push('PROVIDER_ENCRYPTION_KEY');
...
async function ensurePhase9Schema(){
  await db.execute(sql`ALTER TABLE orders ADD COLUMN IF NOT EXISTS dispatching boolean NOT NULL DEFAULT false`);
  await db.execute(sql`CREATE INDEX IF NOT EXISTS orders_dispatch_queue_idx ON orders(status, dispatching, created_at)`);
}

async function ensureProviderSecurity(){
  if(!process.env.PROVIDER_ENCRYPTION_KEY){ if(isProd) throw new Error('PROVIDER_ENCRYPTION_KEY is required'); return; }
  const rows=await db.select().from(providers);
  for(const p of rows){
    if(!isEncryptedSecret(p.apiKey)){
      await db.update(providers).set({apiKey:encryptSecret(p.apiKey)}).where(eq(providers.id,p.id));
    }
  }
}
...
async function startServer(){
  ...
  try { await ensureProviderSecurity(); } catch (e) { console.error('[startup] provider credential security check failed', e); throw e; }
