...
const SECTIONS_EN = [
  ...
  { h: '2. What we do not collect', b: 'We never ask for or store your wallet PIN — electronic-wallet payments are processed and verified by the payment gateway. We do not ask for your social media account password; you only provide a public link (such as a profile or post URL).' },
  ...
  { h: '4. Third-party services', b: 'We rely on a small number of third-party services to operate: secure account services, payment processing services, and service fulfillment partners (to deliver the services you order). Each of these providers processes only the data necessary to perform its function.' },
  ...
  { h: '7. Security', b: 'Your account is protected by secure authentication. Every wallet transaction is written to an append-only internal ledger. We apply standard security practices including rate limiting and encrypted connections (HTTPS) across the Service.' },
...
const SECTIONS_AR = [
  ...
  { h: '٢. ما لا نجمعه', b: 'نحن لا نطلب أو نخزن الرقم السري للمحفظة أبداً — تتم معالجة مدفوعات المحافظ الإلكترونية والتحقق منها عبر بوابة الدفع. لا نطلب كلمة مرور حسابك على مواقع التواصل الاجتماعي؛ تقدم فقط رابطاً عاماً (مثل رابط ملفك الشخصي أو منشور).' },
  ...
  { h: '٤. خدمات الطرف الثالث', b: 'نعتمد على عدد قليل من الخدمات الخارجية للتشغيل: خدمات حماية الحساب، وخدمات معالجة الدفع، وشركاء تنفيذ الخدمات (لتنفيذ الخدمات التي تطلبها). كل من هذه الخدمات يعالج فقط البيانات اللازمة لأداء وظيفته.' },
  ...
  { h: '٧. الأمان', b: 'حسابك محمي بآليات مصادقة آمنة. كل معاملة في المحفظة تُسجل في سجل داخلي دائم. نطبق ممارسات أمان قياسية تشمل تحديد معدل الطلبات واتصالات مشفرة (HTTPS) في كل أنحاء الخدمة.' },
