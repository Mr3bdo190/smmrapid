    ...
    <section className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-sm p-5">
      ...
      {servicesQ.isLoading ? <div className="py-12 text-center text-gray-500 dark:text-gray-400">{t('common.loading')}</div> : servicesQ.isError ? <div className="py-10 text-center"><p className="text-red-600 font-semibold mb-3">Failed to load services.</p><button className="btn-primary" onClick={() => servicesQ.refetch()}>{t('common.refresh')}</button></div> : <>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div><label className="label-primary dark:text-gray-300">2. Service / الخدمة</label><select className="input-primary h-12 dark:bg-slate-700 dark:border-slate-600 dark:text-gray-200" disabled={!categoryId} value={serviceId} onChange={e => chooseService(e.target.value)}><option value="">{categoryId ? 'Select service' : 'Select category first'}</option>{visibleServices.map((s: any) => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div>
        </div>
        {categoryId && <div className="mt-4 flex flex-col md:flex-row gap-3"><div className="relative flex-1"><Search className="absolute left-3 top-3 w-4 h-4 text-gray-400 dark:text-gray-500" /><input className="input-primary pl-9 dark:bg-slate-700 dark:border-slate-600 dark:text-gray-200" placeholder={t('newOrder.searchInCategory')} value={search} onChange={e => setSearch(e.target.value)} /></div><span className="text-sm text-gray-500 dark:text-gray-400 self-center">{visibleServices.length} service(s) in this category</span></div>}
      </>}
    </section>

    {selectedService && <section className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-sm overflow-hidden">
      <div className="p-6 border-b bg-gray-50 dark:bg-slate-700/50">
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
          ...
          <button type="button" onClick={() => toggleFavorite(selectedService.id)} className="btn-secondary shrink-0">
            <Star className={`w-4 h-4 ${favorites.includes(selectedService.id) ? 'fill-yellow-400 text-yellow-500' : ''}`}/>
            {favorites.includes(selectedService.id) ? 'Favorite' : 'Add favorite'}
          </button>
        </div>
      </div>
      <div className="p-6 space-y-6">
        <div className="grid grid-cols-2 lg:grid-cols-6 gap-3">
          <div className="p-4 rounded-xl bg-indigo-50 dark:bg-indigo-900/20">
            <span className="text-xs text-gray-500 dark:text-gray-400">{singleUnit ? 'Price / item' : 'Rate / 1K'}</span>
            <b className="block text-lg text-indigo-700 dark:text-indigo-300">{num(selectedService.pricePer1k).toFixed(4)} {currency}</b>
          </div>
          ...
          <div className="p-4 rounded-xl bg-emerald-50 dark:bg-emerald-900/20">
            ...
            <b className="block text-emerald-700 dark:text-emerald-300">{selectedService.refillable ? 'Available' : 'No'}</b>
          </div>
          <div className="p-4 rounded-xl bg-blue-50 dark:bg-blue-900/20">
            <span className="text-xs text-gray-500 dark:text-gray-400">Cancel</span>
            <b className="block text-blue-700 dark:text-blue-300">{selectedService.cancelable ? 'Available' : 'No'}</b>
          </div>
          <div className="p-4 rounded-xl bg-gray-50 dark:bg-slate-700">
        ...
        <form onSubmit={e => { e.preventDefault(); if (!validQty) return notify.error(singleUnit ? 'This service accepts exactly 1 item.' : t('newOrder.quantityRange', { min: selectedService.minQuantity, max: selectedService.maxQuantity })); if (totalPrice > num(dbUser?.balance)) return notify.error(t('newOrder.insufficientBalance')); order.mutate(); }} className="grid grid-cols-1 lg:grid-cols-[1fr_1fr_260px] gap-4 items-end border-t pt-5 border-gray-200 dark:border-slate-700">
          <div>
            <label className="label-primary dark:text-gray-300">{singleUnit ? 'Email / Account / Required data' : 'Link'}</label>
            <input required type="text" value={link} onChange={e => setLink(e.target.value)} className="input-primary dark:bg-slate-700 dark:border-slate-600 dark:text-gray-200" placeholder={singleUnit ? 'Enter the email, account ID, or required data' : 'https://...'} />
          </div>
          {singleUnit ? <div className="rounded-xl border border-indigo-100 dark:border-indigo-900 bg-indigo-50/60 dark:bg-indigo-900/10 p-4">
            <div className="text-xs text-gray-500 dark:text-gray-400">Quantity</div>
            <div className="text-xl font-black text-indigo-700 dark:text-indigo-300 mt-1">1 item — fixed</div>
            <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">This service is sold as one package/item. The price above is charged once.</div>
          </div> : <div>
          ...
          <div className="rounded-xl bg-gray-950 text-white p-4">
            <div className="text-xs text-gray-400">{t('newOrder.estimatedCharge')}</div>
            <div className="text-2xl font-black mt-1">{totalPrice.toFixed(4)} {currency}</div>
            <div className="text-xs mt-2">{t('common.balance')}: {num(dbUser?.balance).toFixed(4)} {currency}</div>
            <div className="mt-2 text-xs">{validQty ? <span className="text-emerald-400 flex items-center gap-1"><CheckCircle2 className="w-3 h-3" /> Valid quantity</span> : <span className="text-amber-300">Enter a quantity within the limits</span>}</div>
          </div>
