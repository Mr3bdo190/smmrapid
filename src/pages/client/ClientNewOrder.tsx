  ...
  return <div className="space-y-6 max-w-7xl mx-auto" dir="auto">
    ...
    {selectedService && <section className="bg-white rounded-2xl border shadow-sm overflow-hidden">
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="rounded-2xl border border-indigo-100 bg-indigo-50/40 p-5"><div className="flex gap-2 items-center font-bold text-gray-900"><Info className="w-4 h-4 text-indigo-600" /> Service information</div><p className="text-sm text-gray-700 mt-3 whitespace-pre-wrap leading-7">{serviceDetails(selectedService)}</p><div className="grid grid-cols-2 md:grid-cols-3 gap-2 mt-4 text-xs"><div className="rounded-xl bg-white border p-3"><span className="text-gray-500">Refill</span><b className="block mt-1">{selectedService.refillable ? 'Available' : 'Not available'}</b></div><div className="rounded-xl bg-white border p-3"><span className="text-gray-500">Cancellation</span><b className="block mt-1">{selectedService.cancelable ? 'Available' : 'Not available'}</b></div><div className="rounded-xl bg-white border p-3"><span className="text-gray-500">Limits</span><b className="block mt-1">{Number(selectedService.minQuantity).toLocaleString()} – {Number(selectedService.maxQuantity).toLocaleString()}</b></div></div></div>
