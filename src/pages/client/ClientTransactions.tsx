...
export default function ClientTransactions() {
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 dark:text-gray-100">{t('nav.transactions')}</h2>
          ...
          <table className="min-w-full divide-y divide-gray-100 dark:divide-gray-700">
            ...
            <tbody className="bg-white dark:bg-slate-800 divide-y divide-gray-100 dark:divide-gray-700">
              ...
              {transactions.map((p: any) => (
                <tr key={p.id}>
                  <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300 dark:text-gray-300">{new Date(p.createdAt).toLocaleDateString()}</td>
                  <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300 dark:text-gray-300">{p.description}</td>
                  ...
                  <td className="px-6 py-4 text-sm"><span className="px-2 py-1 rounded w-fit text-xs bg-gray-100 dark:bg-slate-700 text-gray-700 dark:text-gray-300 dark:text-gray-300">{p.type}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
